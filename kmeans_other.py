...
mean_mat[0:1,:]=data[210,:-1]
mean_mat[1:2,:]=data[53,:-1]
...
for cell in data:
	...
	if ind==1:
		c2=c2+1	
	if label[count]==2:
		conf_mat[ind,0]=conf_mat[ind,0]+1
	if label[count]==4:
		conf_mat[ind,1]=conf_mat[ind,1]+1
...
for i in range(k):	
	if i==0:
		fi=2*max_array[0,i]/(total[i]+c1)
	if i==1:
		fi=2*max_array[0,i]/(total[i]+c2)
	f=f+fi
print "F-measure:",f/k
...
wnout=0
wnout=wnout+max_array[0,0]*max_array[0,1]
#print winn,wnout
